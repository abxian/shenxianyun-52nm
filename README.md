...
- Managed traffic uses a persisted cumulative idempotency counter. The latest observed totals survive an app crash without replaying accepted bytes. The first local check runs after 5–30 seconds; an idle counter is rechecked locally after 15–30 seconds, while successful reports return to a jittered 5–6 minute cycle. New traffic pulls a distant success timer forward without bypassing the 30-second active minimum interval.
- A traffic attempt has a 15-second total deadline covering connection, response headers, and response body. Timeout aborts the underlying Tauri request, releases the in-flight gate, and retries the same idempotent sequence with backoff starting near 15 seconds; server `Retry-After` always wins. Stop, account switch, and core shutdown abort the old attempt, so a late response cannot restart its scheduler.
